...
实现上，本实验对原版按 run 逐个模拟的流程进行了 NumPy 向量化重写，在每个 time step 批量更新 2000 个独立 Bandit。它属于实现效率和可复现流程的工程改进，epsilon-greedy 算法本身没有改变，详见[完整实验记录](bandit-experiment.md#相对原版的实现改进)。
