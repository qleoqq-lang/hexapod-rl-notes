...
原版 `ten_armed_testbed.py` 的 `simulate()` 按照“算法、run、time step”逐层循环，每次只推进一个 Bandit 实例。本实验在不改变 epsilon-greedy 和样本平均更新规则的前提下，对 `runs` 维度进行了 NumPy 向量化重写：

```text
q_true.shape = (2000, 10)
q_estimates.shape = (2000, 10)
```

在每个 time step 中，脚本会批量完成 2000 个独立 Bandit 的动作选择、reward 采样、最优动作判断和 Q value 更新，只保留具有时序依赖、无法直接消除的 time-step 循环。Q value 相同时仍然随机选择并列最优动作，与原版策略语义保持一致。

这一改动减少了 Python 层对 `runs` 的逐个循环，也方便四种 epsilon 共享同一批 `q_true` 测试环境，并使用显式随机种子复现实验。因此它应表述为**向量化实现与可复现实验流程的工程改进**，而不是新的 Bandit 或 epsilon-greedy 算法。当前尚未进行原版与向量化版本的正式运行时间基准测试，因此不宣称具体加速倍数。
...
- [x] 根据正式结果填写第 8、10 节。
- [x] 用另一个随机种子重复实验，检查结论是否稳定。
- [x] 进入 Chapter 4 GridWorld 与 value iteration。
